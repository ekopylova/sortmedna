...
/*! @fn largest()
    ...
    @return 'a' goes before 'b' if a.first > b.first, otherwise 'a'
*/
              ...
              for ( uint32_t win_num = 0; win_num < numwin; win_num++ )
              {
                ...
                if ( win_num == numwin-1 )
                {
                  ...
                  if ( readhit >= (uint32_t)seed_hits_gv )
                  {
                    ...
                    // STEP 3: find the top min_lis_gv best LIS amongst all candidate
                    // reference sequences
                    uint32_t lis_cutoff = 0;
                    if ( min_lis_gv > -1 )
                    {
                      set<uint32_t> top_LIS;

                      for ( uint32_t k = 0; k < most_frequent_seq.size(); k++ )
                      {
                        max_seq = most_frequent_seq[k].second; 

                        // collect all the genome positions belonging to the
                        // reference candidate from the table of positions
                        ...
                        // length equal to |read| which have at least ratio hits
                        vector<mypair>::iterator it3 = hits_on_genome.begin();
                        deque<mypair> vi_read;
                        deque<mypair>::iterator deq;
                                              
                        // run a sliding window of read's length across
                        ...
                        while ( it3 != hits_on_genome.end() )
                        {
                          ...
                          if ( push && (vi_read.size() >= (uint32_t)seed_hits_gv) )
                          {
                      ...
                      }//~for every candidate reference sequence

                      // store only the LIS length of the top min_lis_gv
                      // LIS
                      uint32_t min_lis_to_keep = min_lis_gv;
                      set<uint32_t>::reverse_iterator it_r;
                      for ( it_r = top_LIS.rbegin(); it_r != top_LIS.rend(); ++it_r )
                      {
                        if ( --min_lis_to_keep == 0 )
                        {
                          lis_cutoff = *it_r;
                          break;
                        }
                    ...
                    // STEP 4: for each reference sequence candidate find the LIS
                    // and perform SW alignment if the LIS is greater or equal to
                    // the lis_cutoff (computed in the previous step) 
                    for ( uint32_t k = 0; k < most_frequent_seq.size(); k++ )
                    {
                      ...
                      max_seq = most_frequent_seq[k].second; 
#ifdef debug_align
                      cout << "\t\t\t\t" << k << "/" << most_frequent_seq.size() << endl;
                      cout << "\t\t\t\tmax_occur = " << max_occur << endl;
                      cout << "\t\t\t\tmax_seq = " << max_seq << endl;
                      cout << "\t\t\t\tnumseq_part = " << numseq_part << endl;
                      cout << "\t\t\t\tindex size for reference_seq = " << (numseq_part<<1) << endl;
                      cout << endl;
#endif                             
